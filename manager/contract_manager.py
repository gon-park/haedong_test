    ...
    def send_order(self, info):
        ...
        while idx < len(self.contracts[info[종목코드]]):
            ...
            if contract.매도수구분 != info[매도수구분]:
                profit = (contract.체결표시가격 - info[가격]) * subject.info[info[종목코드][틱가치]] if contract.매도수구분 is 신규매도 else (info[가격] - contract.체결표시가격) * subject.info[info[종목코드][틱가치]]
                info[수량] -= 1
        ...
        for i in range(0, info[수량]):
            ...
            contract.체결표시가격 = info[가격] + subject.info[contract.종목코드][단위] if contract.매도수구분 is 신규매수 else info[가격] - subject.info[contract.종목코드][단위]
